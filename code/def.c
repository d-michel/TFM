...
void timebreak(time_t *t_0, time_t *t_1, char progress_message[256]) {
	time_t t_2 = time(NULL);
	struct tm *local_time = localtime(&t_2);
	printf("%s\n %s\n Timelapse from beggining: %ld seconds\n Timelapse from previous breakpoint: %ld seconds\n\n", asctime(local_time), progress_message, t_2-*t_0, t_2-*t_1);
	*t_1 = t_2;
}

void remaining_time(float N, float gamma) {
	float a = 0.03801*pow(N, 2) - 0.1794*N + 0.3219;
	float b = 0.005377*N + 0.01694;
	float rtime = a*exp(b*gamma);
	printf("N = %.0f , g = %.0f	Average remaining time: %.0f seconds\n", N, gamma, rtime);
}
...
tsmatrix * matrix_matrix_product(tsmatrix *matrix1, tsmatrix *matrix2) {
	...
	tvector * array1 = init_vector(matrix1->nrow*matrix1->ncol);
	tvector * array2 = init_vector(matrix2->nrow*matrix2->ncol);
	for (int i = 0; i < matrix1->nrow; i++) {
		for (int j = 0; j < matrix2->ncol; j++) {
			array1->data[i*matrix1->nrow+j] = matrix1->data[i][j];
			array2->data[i*matrix2->nrow+j] = matrix2->data[i][j];
		}
	}
	
	tvector * array_result = init_vector(matrix1->nrow*matrix2->ncol);
	...
	*/
	...
	cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, matrix_result->nrow, matrix_result->ncol,
				matrix1->ncol, 1, array1->data, matrix1->nrow, array2->data, matrix2->nrow, 0,
				array_result->data, matrix_result->nrow);
	
	for (int i = 0; i < matrix_result->nrow; i++) {
		for (int j = 0; j < matrix_result->ncol; j++) {
			matrix_result->data[i][j] = array_result->data[i*matrix_result->nrow+j];
		}
	}
...
*/
	...
	tsmatrix * Ematrix_matrix = init_square_matrix(Ematrix->length, Ematrix->length);
	for (int i = 0; i < Ematrix->length; i++) {
		Ematrix_matrix->data[i][i] = Ematrix->data[i];
	}
	matrix_aux = matrix_matrix_product(Ematrix_matrix, P_inverse);
...
/*	tvector * matrix_array = init_vector(matrix->nrow*(matrix->nrow + 1)/2);
	int k = 0;
	for (int i = 0; i < matrix->nrow; i++) {							// for each row
		for (int j = 0; j < matrix->ncol; j++) {						// and for each column
			matrix_array->data[k] = matrix->data[i][j];					// insert matrix elements into the
			k++;															// float-array matrix_array->data
		}
	}
	// cblas_ssymv(cblas_layout, cblas_uplo, n, alpha, a, lda, x, incx, beta, y, incy);
	cblas_ssymv(CblasRowMajor, CblasUpper, matrix->ncol, 1, matrix_array->data, matrix->ncol, array, 1, 0, result, 1);
*/	
	return result;
}
...
tsmatrix * density_of_states(tspectrum *spectrum, int nwindows) {
	float window_width = (spectrum->eigvals[spectrum->length-1] - spectrum->eigvals[0])/((float) nwindows);
	tsmatrix * density = init_square_matrix(nwindows, 2);
	
	int j = 0;
	for (int i = 0; i < density->nrow; i++) {
		density->data[i][0] = spectrum->eigvals[0] + (i+1)*window_width;
		while ((j < spectrum->length) && (spectrum->eigvals[j] < density->data[i][0])) {
			density->data[i][1] += 1;
			j++;
		}
	}
...
tsmatrix * fast_window_analysis(tspectrum *spectrum, tvector *min_ratio, float num_windows) {
	tvector * end_points = init_vector(num_windows+1);
	float length_window = (spectrum->eigvals[spectrum->length-1] - spectrum->eigvals[0])/num_windows;
	...
	for (int i = 0; i < num_windows; i++) {
		while (spectrum->eigvals[j] < (spectrum->eigvals[0] + (i+1)*length_window) && j < spectrum->length) {
			j++;
		}
		end_points->data[i+1] = j;
		mean_window_min_ratio->data[i][0] = mean_vector(min_ratio, end_points->data[i], end_points->data[i+1]);
		mean_window_min_ratio->data[i][1] = spectrum->eigvals[(int)end_points->data[i]];
		mean_window_min_ratio->data[i][2] = spectrum->eigvals[(int)end_points->data[i+1]];
	}
...
float * op_sigma(tmixstate *mix_state) {
	...
	for (int i = 0; i < mix_state->nstates; i++) {
		sigma[0] += pow(mix_state->width[i], 2)*mix_state->data[i]->n[0];
		sigma[1] += pow(mix_state->width[i], 2)*mix_state->data[i]->n[1];
		sigma[2] += pow(mix_state->width[i], 2)*mix_state->data[i]->n[2];
	}
...
tvector * peres_lattice_a_plus_ad(tbasis *basis, tspectrum *spectrum) {
	tsmatrix * Hmatrix_a_plus_ad = H_a_plus_ad(basis);
	tvector * On_a_plus_ad = init_vector(spectrum->length);
	tvector * aux_vector = init_vector(spectrum->eigvectors->ncol);
	
	for (int i = 0; i < spectrum->length; i++) {
		aux_vector->data = matrix_array_product(Hmatrix_a_plus_ad, spectrum->eigvectors->data[i]);
		On_a_plus_ad->data[i] = scalar_product(aux_vector->data, spectrum->eigvectors->data[i], aux_vector->length);
	}
...
float critical_energy(tspectrum *spectrum, tvector *C_expected_value, float tol) {
	//tsmatrix * Ec = init_square_matrix(3,2);
	int j = 0;
	while ((fabs(1. - fabs(C_expected_value->data[j])) < tol) && (j < C_expected_value->length)) {
		j++;
	}
	// C(j - 1) es el último que cumple la tolerancia
	float m = (fabs(C_expected_value->data[j-1]) - fabs(C_expected_value->data[j]))/(spectrum->eigvals[j-1] - spectrum->eigvals[j]);
	float n = fabs(C_expected_value->data[j-1]) - m*spectrum->eigvals[j-1];
	float Ec = (0.95 - n)/m;
